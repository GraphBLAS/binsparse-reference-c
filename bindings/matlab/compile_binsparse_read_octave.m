...
function compile_binsparse_read_octave()
% COMPILE_BINSPARSE_READ_OCTAVE - Quick Octave compilation for binsparse_read
%
% This script compiles just the binsparse_read MEX function using mkoctfile
% with proper library linking for Octave.

fprintf('Compiling binsparse_read MEX function for Octave...\n');

% Check if we're in Octave
if ~(exist('OCTAVE_VERSION', 'builtin') ~= 0)
    warning('This script is designed for Octave. For MATLAB, use compile_binsparse_read.m');
end
...
cmd = sprintf('mkoctfile --mex -fPIC -I%s binsparse_read.c -Wl,--whole-archive %s %s -Wl,--no-whole-archive -lhdf5_serial', ...
